...
export function StudentForm({
  ...
  useEffect(() => {
    if (!initialData && generateNewAdmissionNumber && !formData.admission_number) {
      setIsGeneratingAdmission(true)
      generateNewAdmissionNumber().then(adm => {
        if (adm) {
          setFormData(prev => ({ ...prev, admission_number: adm }))
        }
      }).finally(() => {
        setIsGeneratingAdmission(false)
      })
    }
  }, [initialData, generateNewAdmissionNumber])
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input 
              ...
              value={isGeneratingAdmission ? "Generating..." : formData.admission_number} 
              onChange={handleChange} 
              readOnly={!initialData || isGeneratingAdmission}
              className={(!initialData || isGeneratingAdmission) ? "bg-muted text-muted-foreground" : ""}
            />
          </div>
        </div>
